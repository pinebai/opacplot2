...
class OpgSesame:
    ...
    CHAR_LINE_LEN = 80
    # TODO add option for words per line
    WORDS_PER_LINE = 5
    #WORDS_PER_LINE = 4
    ...
    def readEntries(self,nentries):
        ...
        print(nlines, self.WORDS_PER_LINE)
    ...
    def toEosDict(self, Znum=None, Anum=None, 
        ...
        # RoksK's table doesn't have ioncc curve so we're just going to merge
        # ele and ion (304 & 305)
        if not qeos:
            opp_ses_data = opp.utils.EosMergeGrids(
                                        opp_ses_data, intersect=['ele', 'ion'],                                                      filter_dens=lambda x: (x>filter_dens),
